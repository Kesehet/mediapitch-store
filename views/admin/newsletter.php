...
<section class="admin-card">
  <div class="admin-grid stats-grid" style="margin-bottom:20px">
    ...
    <div class="stat-card"><span>Needs review</span><strong><?= number_format((int)($stats['risky']??0)+(int)($stats['invalid_count']??0)) ?></strong></div>
  ...
  <form method="get" action="<?= e(url('admin/newsletter')) ?>" class="admin-filter-row" style="display:flex;gap:10px;flex-wrap:wrap;align-items:end;margin-bottom:18px">
    ...
    <label>Validation<select name="validation"><option value="all">All</option><option value="clean" <?= $validation==='clean'?'selected':'' ?>>Clean</option><option value="risky" <?= $validation==='risky'?'selected':'' ?>>Risky</option><option value="invalid" <?= $validation==='invalid'?'selected':'' ?>>Invalid</option><option value="unknown" <?= $validation==='unknown'?'selected':'' ?>>Unknown</option><option value="not_checked" <?= $validation==='not_checked'?'selected':'' ?>>Not checked</option></select></label>
