    ...
    <form method="post" action="<?= e(url('admin/brands/save')) ?>" class="admin-form">
      ...
      <?php if(!empty($mediaItems)): ?>
        <label>Choose logo from media library
          <select id="brand-logo-picker">
            <option value="">— Select uploaded image —</option>
            <?php foreach($mediaItems as $media): $mediaUrl=url(ltrim((string)$media['file_path'],'/')); ?>
              <option value="<?= e($mediaUrl) ?>" <?= ($brand['logo_url']??'')===$mediaUrl?'selected':'' ?>><?= e($media['original_name']) ?><?= !empty($media['alt_text'])?' — '.e($media['alt_text']):'' ?></option>
            <?php endforeach; ?>
          </select>
      ...
      <div class="form-actions">
        <button class="primary-button" type="submit">Save brand</button>
        <?php if ($editing): ?><a class="secondary-button" href="<?= e(url('admin/brands')) ?>">Cancel</a><?php endif; ?>
      </div>
    </form>
  </section>
  ...
  <section class="panel">
    ...
    <?php else: ?>
      <div class="table-wrap"><table class="data-table">
        <thead><tr><th>Brand</th><th>Slug</th><th>Website</th><th></th></tr></thead>
        <tbody>
...
<script>
(function(){const picker=document.getElementById('brand-logo-picker'),input=document.getElementById('brand-logo-url');if(picker&&input)picker.addEventListener('change',()=>{if(picker.value)input.value=picker.value;});})();
</script>
