...
<label class="span-2">Amazon URL<input type="url" id="product-amazon-url" name="amazon_url" value="<?= e($p['amazon_url'] ?? '') ?>"></label><label class="span-2">Affiliate URL<input type="url" id="product-affiliate-url" name="affiliate_url" value="<?= e($p['affiliate_url'] ?? '') ?>"></label>
<label class="span-2">SEO title<input name="seo_title" maxlength="255" value="<?= e($p['seo_title'] ?? '') ?>"></label>
<label class="span-2">Meta description<textarea name="meta_description" maxlength="320" rows="3"><?= e($p['meta_description'] ?? '') ?></textarea></label>
<label class="span-2">Canonical URL<input type="url" name="canonical_url" value="<?= e($p['canonical_url'] ?? '') ?>"></label>
<label class="check span-2"><input type="checkbox" name="robots_index" value="1" <?= !isset($p['robots_index'])||!empty($p['robots_index'])?'checked':'' ?>> Allow search engines to index this product</label>
<label class="span-2">Short description<textarea name="short_description" rows="3"><?= e($p['short_description'] ?? '') ?></textarea></label><label class="span-2">Full description<textarea name="full_description" rows="6"><?= e($p['full_description'] ?? '') ?></textarea></label><label>Features <small>one per line</small><textarea name="features" rows="6"><?= e($decode($p['features_json'] ?? null)) ?></textarea></label><label>Pros <small>one per line</small><textarea name="pros" rows="6"><?= e($decode($p['pros_json'] ?? null)) ?></textarea></label><label>Cons <small>one per line</small><textarea name="cons" rows="6"><?= e($decode($p['cons_json'] ?? null)) ?></textarea></label><label>Editorial notes<textarea name="editorial_notes" rows="6"><?= e($p['editorial_notes'] ?? '') ?></textarea></label></div>
