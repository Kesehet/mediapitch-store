...
if($method==='POST'){
    ...
    if($id>0){if($action==='delete')$repo->delete($id);elseif($action==='activate')$repo->setStatus($id,'active');elseif($action==='unsubscribe')$repo->setStatus($id,'unsubscribed');}
    header('Location: /admin/newsletter.php?success='.rawurlencode('Subscriber updated.'));exit;
}
$query=trim((string)($_GET['q']??''));$status=(string)($_GET['status']??'all');$rows=$repo->all($query,$status);
if(($_GET['export']??'')==='csv'){header('Content-Type: text/csv; charset=utf-8');header('Content-Disposition: attachment; filename="mediapitch-newsletter-subscribers-'.date('Y-m-d').'.csv"');$out=fopen('php://output','w');fputcsv($out,['Email','Status','Source','Subscribed at','Unsubscribed at']);foreach($rows as $row)fputcsv($out,[$row['email'],$row['status'],$row['source'],$row['subscribed_at'],$row['unsubscribed_at']]);fclose($out);exit;}
