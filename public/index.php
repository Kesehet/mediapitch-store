...
try {
    ...
    if (str_starts_with($path, '/admin/brands')) {
        $brandAdmin = new BrandAdminController(new BrandRepository());
        if ($brandAdmin->handle($method, $path)) exit;
    }
    ...
    if($method==='GET' && !str_starts_with($path,'/api/') && $path!=='/health'){
        try{$redirect=$redirectRepo->resolve($path);}catch(Throwable $redirectError){$redirect=null;if((bool)env('APP_DEBUG',false))error_log('Redirect lookup failed: '.$redirectError->getMessage());}
        if($redirect){$destination=(string)$redirect['to_url'];if(str_starts_with($destination,'/'))$destination=url(ltrim($destination,'/'));header('Location: '.$destination,true,(int)$redirect['status_code']);exit;}
    }

    if ($method === 'GET' && $path === '/') {
        $categories=[];$products=[];$guides=[];$articles=[];$comparisons=[];$deals=[];$dealsTitle='Deals worth a look';$databaseAvailable=true;
        try {
            $categories=$catalog->featuredCategories();$products=$catalog->featuredProducts();$guides=$catalog->latestContent('buying_guide');$articles=$catalog->latestContent('blog');$comparisons=$comparisonCatalog->latest(3);
            $merch=$merchandisingRepo->homepage();if(!empty($merch['featured']))$products=$merch['featured'];$deals=$merch['deals']??[];$dealsTitle=(string)($merch['deals_title']??$dealsTitle);
        } catch (Throwable $databaseError) {$databaseAvailable=false;if ((bool) env('APP_DEBUG', false)) error_log('MediaPitch Store homepage database error: '.$databaseError->getMessage());}
        View::render('home',['pageTitle'=>'MediaPitch Store — Smart Product Discovery','metaDescription'=>'Independent buying guides, product comparisons and recommendations from MediaPitch.','categories'=>$categories,'products'=>$products,'guides'=>$guides,'articles'=>$articles,'comparisons'=>$comparisons,'deals'=>$deals,'dealsTitle'=>$dealsTitle,'databaseAvailable'=>$databaseAvailable]);exit;
    }
    ...
    if ($method === 'GET' && preg_match('#^/blog/([a-z0-9-]+)$#i',$path,$matches)) {
        $post=$contentRepo->publishedPostBySlug($matches[1]);if(!$post){http_response_code(404);View::render('404',['pageTitle'=>'Article not found','metaDescription'=>'']);exit;}
        View::render('article',['pageTitle'=>($post['seo_title'] ?: $post['title']).' — MediaPitch','metaDescription'=>(string)($post['meta_description'] ?: $post['excerpt']),'canonicalUrl'=>$post['canonical_url'] ?: url('blog/'.$post['slug']),'robotsIndex'=>(bool)$post['robots_index'],'post'=>$post]);exit;
    }

    if ($method === 'GET' && $path === '/comparisons') {View::render('comparisons-index',['pageTitle'=>'Product Comparisons — MediaPitch Store','metaDescription'=>'Compare products side by side with MediaPitch specifications, scores, prices and editorial verdicts.','canonicalUrl'=>url('comparisons'),'comparisons'=>$comparisonCatalog->published()]);exit;}
    if ($method === 'GET' && preg_match('#^/compare/([a-z0-9-]+)$#i',$path,$matches)) {
        $comparison=$comparisonRepo->publishedBySlug($matches[1]);if(!$comparison){http_response_code(404);View::render('404',['pageTitle'=>'Comparison not found','metaDescription'=>'']);exit;}
        View::render('comparison',['pageTitle'=>($comparison['seo_title'] ?: $comparison['title']).' — MediaPitch','metaDescription'=>(string)($comparison['meta_description'] ?: $comparison['excerpt']),'canonicalUrl'=>$comparison['canonical_url'] ?: url('compare/'.$comparison['slug']),'robotsIndex'=>(bool)$comparison['robots_index'],'comparison'=>$comparison]);exit;
    }

    if ($method === 'GET' && preg_match('#^/review/([a-z0-9-]+)$#i',$path,$matches)) {
        $review=$reviewRepo->publishedBySlug($matches[1]);if(!$review){http_response_code(404);View::render('404',['pageTitle'=>'Review not found','metaDescription'=>'']);exit;}
    ...
    if ($method === 'GET' && $path === '/search') {
        ...
        if($query!==''&&$page===1){try{$searchRepo->recordQuery($query,$categoryId?:null,(int)($results['result_count']??0));}catch(Throwable $searchLogError){if((bool)env('APP_DEBUG',false))error_log('Search analytics write failed: '.$searchLogError->getMessage());}}
        View::render('search',['pageTitle'=>$query!==''?'Search: '.$query.' — MediaPitch':'Search — MediaPitch','metaDescription'=>'Search MediaPitch products, categories, buying guides, comparisons and articles.','query'=>$query,'selectedCategory'=>$categoryId,'searchCategories'=>$searchRepo->categoryOptions(),'results'=>$results]);exit;
    }
    ...
    if ($method === 'GET' && preg_match('#^/guide/([a-z0-9-]+)$#i',$path,$matches)) {
        $guide=$catalog->buyingGuideBySlug($matches[1]);if(!$guide){http_response_code(404);View::render('404',['pageTitle'=>'Guide not found','metaDescription'=>'']);exit;}
        View::render('guide',['pageTitle'=>($guide['seo_title'] ?: $guide['title']).' — MediaPitch','metaDescription'=>(string)($guide['meta_description'] ?: $guide['excerpt']),'canonicalUrl'=>$guide['canonical_url'] ?: url('guide/'.$guide['slug']),'robotsIndex'=>(bool)$guide['robots_index'],'guide'=>$guide]);exit;
    }
    ...
    if ($method === 'GET' && $path === '/health') {header('Content-Type: application/json; charset=utf-8');try{Database::connection()->query('SELECT 1');echo json_encode(['status'=>'ok','database'=>'ok'],JSON_THROW_ON_ERROR);}catch(Throwable){http_response_code(503);echo json_encode(['status'=>'degraded','database'=>'unavailable'],JSON_THROW_ON_ERROR);}exit;}
