...
final class NotificationMailer
{
    public function sendDraftReady(array $settings,array $draft,array $job,array $sources): void
    {
        ...
        $base=((string)($job['content_type']??'blog'))==='buying_guide'?'admin/guides/':'admin/blog/';
        $reviewUrl=url($base.(int)($job['content_id']??0).'/edit');
        $sourceLines=[];
        ...
        $headers=['Content-Type: text/plain; charset=UTF-8'];if($from!=='')$headers[]='From: MediaPitch Store <'.$from.'>';
        $subject='[MediaPitch] New AI draft ready: '.$title;
        foreach($recipients as $recipient)if(!@mail($recipient,$subject,$body,implode("\r\n",$headers)))throw new RuntimeException('Could not send AI draft notification email to '.$recipient.'.');
    }
}
