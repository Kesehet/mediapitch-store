...
final class ProductAuthoring
{
    ...
    public function archive(int $productId): void
    {
        ...
        if($stmt->rowCount()===0) throw new RuntimeException('Product not found or already archived.');
    }
    ...
    public function duplicate(int $productId): int
    {
        ...
        $baseSlug=$this->slugify((string)$source['slug'].'-copy');
        $slug=$baseSlug;$suffix=2;
        $check=$db->prepare('SELECT COUNT(*) FROM products WHERE slug=:slug');
        while(true){$check->execute(['slug'=>$slug]);if((int)$check->fetchColumn()===0)break;$slug=$baseSlug.'-'.$suffix++;}
        ...
        try{
            $insert=$db->prepare(
                'INSERT INTO products (category_id,brand_id,asin,title,display_title,slug,source,api_marketplace,short_description,full_description,main_image_url,gallery_json,features_json,pros_json,cons_json,price,previous_price,currency,amazon_url,affiliate_url,custom_score,best_for_label,editorial_notes,manual_override_json,last_synced_at,active)
                 VALUES (:category_id,:brand_id,NULL,:title,:display_title,:slug,\'manual\',NULL,:short_description,:full_description,:main_image_url,:gallery_json,:features_json,:pros_json,:cons_json,:price,:previous_price,:currency,NULL,NULL,:custom_score,:best_for_label,:editorial_notes,NULL,NULL,0)'
            );
            $insert->execute([
                'category_id'=>$source['category_id'],'brand_id'=>$source['brand_id'],
                'title'=>'Copy of '.$source['title'],'display_title'=>$source['display_title']?'Copy of '.$source['display_title']:null,
                'slug'=>$slug,'short_description'=>$source['short_description'],'full_description'=>$source['full_description'],
                'main_image_url'=>$source['main_image_url'],'gallery_json'=>$source['gallery_json'],'features_json'=>$source['features_json'],
                'pros_json'=>$source['pros_json'],'cons_json'=>$source['cons_json'],'price'=>$source['price'],'previous_price'=>$source['previous_price'],
                'currency'=>$source['currency'],'custom_score'=>$source['custom_score'],'best_for_label'=>$source['best_for_label'],'editorial_notes'=>$source['editorial_notes'],
            ]);
            ...
            $copySpecs=$db->prepare(
                'INSERT INTO product_specifications (product_id,specification_definition_id,value_text,value_number,value_boolean)
                 SELECT :new_id,specification_definition_id,value_text,value_number,value_boolean FROM product_specifications WHERE product_id=:source_id'
            );
