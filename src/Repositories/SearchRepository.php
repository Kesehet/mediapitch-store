...
{
    public function search(string $query,int $page=1,int $perPage=12,?int $categoryId=null): array
    {
        ...
        $like='%'.$query.'%';
        $productCategorySql=$categoryId!==null?' AND p.category_id=:category_id':'';
        ...
        $countProducts=$db->prepare(
            'SELECT COUNT(*)
             ...
             WHERE p.active=1'.$productCategorySql.' AND (p.title LIKE :q1 OR p.display_title LIKE :q2 OR p.short_description LIKE :q3 OR b.name LIKE :q4 OR c.name LIKE :q5)'
        );
        if($categoryId!==null)$countProducts->bindValue(':category_id',$categoryId,PDO::PARAM_INT);
        ...
        if($categoryId!==null)$products->bindValue(':category_id',$categoryId,PDO::PARAM_INT);
        ...
        if($categoryId!==null){
            $categories=$db->prepare(
                'SELECT id,name,slug,description,image_url FROM categories
                 WHERE active=1 AND id=:category_id AND (name LIKE :q1 OR description LIKE :q2)
                 ORDER BY sort_order,name LIMIT 12'
            );
            $categories->bindValue(':category_id',$categoryId,PDO::PARAM_INT);
        }else{
            $categories=$db->prepare(
                'SELECT id,name,slug,description,image_url FROM categories
        ...
        $resultCount=$productTotal+count($categoryRows)+count($contentRows);
        return [
    ...
    public function recordQuery(string $query,?int $categoryId,int $resultCount): void
    {
        $query=mb_substr(trim($query),0,255);
        if($query==='')return;
        $stmt=Database::connection()->prepare(
            'INSERT INTO search_queries (query_text,category_id,result_count) VALUES (:query_text,:category_id,:result_count)'
        );
        $stmt->execute([
            'query_text'=>$query,
