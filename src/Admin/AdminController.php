...
final class AdminController
{
    ...
    public function handle(string $method, string $path): bool
    {
        ...
        if ($path === '/admin/account/password' && $method === 'POST') {
            ...
            try {
                ...
                Audit::record('account.password.change','user',(int)$user['id'],'Changed own password');
        ...
        if (str_starts_with($path, '/admin/users')) {
            ...
            if ($path === '/admin/users/save' && $method === 'POST') {
                ...
                try {
                    $current=Auth::user();
                    $savedId=$usersRepo->save($_POST,$existingId,(int)$current['id']);
                    Audit::record($existingId?'user.update':'user.create','user',$savedId,$existingId?'Updated user account':'Created user account',[
                        'name'=>(string)($_POST['name']??''),'email'=>(string)($_POST['email']??''),'role'=>(string)($_POST['role']??''),'active'=>!empty($_POST['active']),
                        'password'=>!empty($_POST['password'])?'[changed]':'[unchanged]',
                    ]);
        ...
        if (str_starts_with($path, '/admin/brands')) {
            ...
            if ($path === '/admin/brands/save' && $method === 'POST') {
                ...
                try {
                    ...
                    $id=$this->repo->saveBrand($_POST,$existingId);
                    Audit::record($existingId?'brand.update':'brand.create','brand',$id,$existingId?'Updated brand':'Created brand',[
                        'name'=>(string)($_POST['name']??''),'slug'=>(string)($_POST['slug']??''),'website_url'=>$website,'logo_url'=>$logo,
                    ]);
        ...
        if (str_starts_with($path, '/admin/products')) {
            ...
            if ($path === '/admin/products/save' && $method === 'POST') {
                ...
                try {
                    ...
                    Audit::record($existingId?'product.update':'product.create','product',$id,$existingId?'Updated product':'Created product',[
            ...
            if ($method === 'POST' && preg_match('#^/admin/products/(\d+)/(archive|restore|duplicate)$#',$path,$m)) {
                ...
                try{
                    ...
                    if($action==='archive'){
                        $actions->archive($id);
                        Audit::record('product.archive','product',$id,'Archived product');
                        $this->setFlash('success','Product archived.'); $this->redirect('/admin/products');
                    }
                    if($action==='restore'){
                        $actions->restore($id);
                        Audit::record('product.restore','product',$id,'Restored product');
                        $this->setFlash('success','Product restored.'); $this->redirect('/admin/products');
                    }
                    $newId=$actions->duplicate($id);
                    Audit::record('product.duplicate','product',$newId,'Duplicated product',['source_product_id'=>$id]);
        ...
        if (str_starts_with($path, '/admin/guides')) {
            ...
            if ($path === '/admin/guides/save' && $method === 'POST') {
                ...
                try {
                    ...
                    Audit::record($existingId?'guide.update':'guide.create','buying_guide',$id,$existingId?'Updated buying guide':'Created buying guide',[
                        'title'=>(string)($_POST['title']??''),'slug'=>$newSlug,'status'=>$status,'category_id'=>!empty($_POST['category_id'])?(int)$_POST['category_id']:null,
    ...
    private function requireContentEditor(): void
    {
